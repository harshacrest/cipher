"""Backtest runner — per-day NautilusTrader BacktestEngine runs."""
...
def run_single_day_engine(date_str: str, config: ATMStraddleSellConfig) -> dict | None:
    ...
    strategy = ATMStraddleSell(config=config)
...
def run_backtest(strategy_name: str = "atm_straddle_sell") -> pd.DataFrame:
    """Run strategy across all available trading days."""
    days = list_trading_days()
    config = ATMStraddleSellConfig()
    trades = []
...
def main():
    strategy_name = "atm_straddle_sell"
