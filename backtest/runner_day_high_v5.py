"""Backtest runner — Day High OTM Sell v5 (true whole-day day high)."""
...
def run_single_day(date_str: str, config: DayHighOTMSellV5Config) -> list[dict]:
    try:
    ...
    strategy = DayHighOTMSellV5(config=config)
...
def run_backtest(strategy_name: str = "day_high_otm_sell_v5", last_n: int = 0,
                 cost_per_rt: float = 0.0) -> pd.DataFrame:
    days = list_trading_days()
    if last_n > 0:
        days = days[-last_n:]

    config = DayHighOTMSellV5Config(cost_per_round_trip_pts=cost_per_rt)
...
def main():
    ...
    strategy_name = "day_high_otm_sell_v5"
