"""Backtest runner — Day High VIX ATM Straddle Sell strategy."""
...
from lib.nautilus_data import VENUE, INR, load_day_data_with_vix
from lib.reporting import generate_report
from strategies.day_high_vix_straddle_sell import DayHighVixStraddleSell, DayHighVixStraddleSellConfig


def run_single_day_engine(date_str: str, config: DayHighVixStraddleSellConfig) -> list[dict]:
    try:
        instruments, ticks = load_day_data_with_vix(
            date_str,
    ...
    if not ticks or len(instruments) < 3:  # spot + vix + at least one option
        return []
    ...
    strategy = DayHighVixStraddleSell(config=config)
...
def run_backtest(strategy_name: str = "day_high_vix_straddle", last_n: int = 0) -> pd.DataFrame:
    days = list_trading_days()
    if last_n > 0:
        days = days[-last_n:]
    config = DayHighVixStraddleSellConfig()
    all_trades = []
...
def main():
    ...
    strategy_name = "day_high_vix_straddle"
