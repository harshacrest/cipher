"""MultiLegDM v4 — same SL profile as v2/v3, but 1s tick resolution.

Same strategy class as v1 (`strategies/multi_leg_dm.MultiLegDM`).
Same SL caps as v2/v3:
  trade_sl_premium       :  -155 pts
  daily_sl_threshold     :  -550 pts

Tick resolution sweep:
  v1, v2  : 30 s resample
  v3      :  5 s resample
  v4      :  1 s resample  ← this run

Why this matters
----------------
Each finer-resolution step exposes the strategy to more brief band/SL
crossings that were previously smoothed away. v3 vs v2 already showed
SPOT_BAND avg PnL collapsing from +7.7 → +0.91 / trade. v4 will show
where the noise floor really is at 1 s — important for honest live-execution
calibration since real polling is ≤1 s.

Output: output/multi_leg_dm_v4/
"""
...
STRATEGY_NAME = "multi_leg_dm_v4"
RESAMPLE = "1s"


def make_v4_config() -> MultiLegDMConfig:
...
def run_backtest(
    ...
    print(
        f"v4 config: trade_sl={config.trade_sl_premium}, "
