"""VWAP SD Straddles V3 backtest runner — time-based entry, trade + daily SL + strike shift."""
...
from strategies.vwap_sd_straddles_v3 import VWAPSDStraddlesV3, VWAPSDStraddlesV3Config


def run_single_day_engine(date_str: str, base_config_kwargs: dict) -> list[dict]:
    try:
    ...
    config = VWAPSDStraddlesV3Config(**base_config_kwargs)
    strategy = VWAPSDStraddlesV3(config=config)
...
def run_backtest(
    strategy_name: str = "vwap_sd_straddles_v3",
    ...
    base_config_kwargs = {
        ...
        "lot_size": 1,
        "num_lots": 1,
        "trade_sl_points": 500.0,
        "daily_sl_points": 1500.0,
        "spot_band_x": 1.5,
    ...
    for day in tqdm(days, desc=f"Backtesting {strategy_name}"):
        trades = run_single_day_engine(day, base_config_kwargs)
...
def main():
    strategy_name = "vwap_sd_straddles_v3"
