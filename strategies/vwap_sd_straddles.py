"""
VWAP SD Straddles Strategy — NautilusTrader
--------------------------------------------
Translation of the Pine Script "Harsha Extrinsic Intrinsic VWAP SDs 15 Straddles".

Core idea
---------
Aggregate 15 straddles spanning ATM ± 350 pts (step 50) into a single
synthetic price `sClose`. This is the total premium of all 30 option legs.
This aggregate behaves as a mean-reverting volatility index intraday.

- baseStrike = ATM at session start (first spot tick rounded to nearest 50)
- legs: 30 instruments (15 CE + 15 PE at strikes baseStrike + 50*{-7..+7})
- sClose = sum of all 30 option mid prices per 1-minute bar

VWAP + SD bands (daily reset, cumulative/expanding since day start):
- VWAP = expanding mean of sClose (volume-unweighted; NIFTY index has no volume)
- stdev = population stdev of sClose since day start
- VAH = VWAP + 1.0 * stdev (upper)
- VAL = VWAP - 1.0 * stdev (lower)

Entry (short the aggregate straddle):
- Minute bar close sClose crosses under VWAP (prior bar was >= VWAP, current < VWAP)
- Time is between 09:21 and 15:00 IST
- No open position

Exit (close all 30 legs simultaneously):
1. Stop Loss: sClose > VWAP + sl_points_above_vwap    [default 500 pts]
2. Forced:    time >= 15:12 IST

There is NO profit target — winners ride to EOD.
"""
...
class VWAPSDStraddlesConfig(StrategyConfig):
    # Time windows (IST)
    session_start_time: str = "09:15:00"  # when baseStrike is locked
    entry_window_start: str = "09:21:00"
    entry_window_end: str = "15:00:00"
    forced_exit_time: str = "15:12:00"
    eod_time: str = "15:25:00"            # hard cutoff

    # Structure
    strike_step: int = 50
    num_strikes_each_side: int = 7         # M7..P7 = 15 strikes total
    lot_size: int = 1
    num_lots: int = 1

    # Signal parameters
    num_sd: float = 1.0                    # SD multiplier for VAH/VAL (display only)
    sl_points_above_vwap: float = 500.0    # SL = VWAP + this many points (absolute)

    # Strike shift: on |spot-entry_spot| > atm_straddle/spot_band_x, roll strikes to new ATM
    spot_band_x: float = 1.5

    # Cross-day state (kept for backward-compat with runner; unused by new rule)
    prev_day_lower_band: float | None = None
...
class VWAPSDStraddles(Strategy):

    def __init__(self, config: VWAPSDStraddlesConfig) -> None:
        ...
        # Base strike + legs (built at session start once)
        self.base_strike: int | None = None
        self.leg_instruments: list[dict] = []  # each: {strike, side, instrument_id, latest_mid, latest_bid, latest_ask}
        self._leg_by_id: dict = {}  # instrument_id -> leg dict (O(1) lookup in on_quote_tick)

        # 1-minute bar state for aggregate straddle — track as UTC ns (integer math, fast)
        self._current_minute_ns: int = 0  # floor of current tick's minute, in UTC ns
        self._bar_sclose: float = 0.0
        self._prev_bar_sclose: float | None = None
        self._prev_bar_vwap: float | None = None
        self._minute_ns = 60_000_000_000  # 1 minute in ns

        # Cumulative VWAP + stdev since day start
        ...
        # Last observed lowerBand (for next-day prev_day_lower_band)
        self.last_lower_band_of_day: float | None = None
        ...
        # Per-trade fill tracking (keyed by instrument_id str)
    ...
    # ---------------------------------------------------------------
    # Session start — lock baseStrike + build legs
    # ---------------------------------------------------------------
    ...
    def _on_session_start(self, event) -> None:
        ...
        # Lock base strike = nearest multiple of step
        ...
        self.log.info(
            ...
            f"legs = 2 x {2 * self.n_side + 1} = {2 * (2 * self.n_side + 1)}"
        )

        # Build leg instrument IDs (15 CE + 15 PE = 30 legs)
        ...
        for offset in range(-self.n_side, self.n_side + 1):
            ...
            for side in ("CE", "PE"):
                ...
                if self.cache.instrument(inst_id) is None:
                    continue  # skip missing strikes gracefully
                leg = {
                    "strike": strike,
                    "side": side,
    ...
    # ---------------------------------------------------------------
    # Tick handler — drives 1-min bar aggregation
    # ---------------------------------------------------------------
    ...
    def on_quote_tick(self, tick) -> None:
        ...
        if inst_id == self.spot_id:
            ...
            # Strike-shift: if spot moved outside atm_straddle/spot_band_x, roll to new ATM
        ...
        # Minute boundary check — integer math on ns (no pd.Timestamp per tick)
        ...
        if self._current_minute_ns == 0:
            ...
            # Previous minute just closed — finalize bar, evaluate signals
            self._close_bar(self._current_minute_ns)
            self._current_minute_ns = tick_minute_ns

    # ---------------------------------------------------------------
    # Bar close: aggregate straddle price, update VWAP/SD, evaluate entry/exit
    # ---------------------------------------------------------------

    def _close_bar(self, bar_minute_ns: int) -> None:
        # Aggregate straddle close = sum of all leg mids
        total = 0.0
        n_valid = 0
        ...
        # Require enough legs loaded (at least ATM ± 3 strikes on each side both sides)
        if n_valid < 10:
            return
        ...
        # Update cumulative VWAP + stdev (volume-unweighted, expanding since day start)
        ...
        # Track last observed lower band (will be yesterday's VAL for next day)
        self.last_lower_band_of_day = self.current_lower_band

        # ---- Exit check (SL only — no profit target; winners ride to forced exit) ----
        if self._fsm == STATE_ACTIVE:
            # SL: sclose > VWAP + sl_points  (absolute, e.g. VWAP + 500)
            sl_level = self.current_vwap + self.sl_points
            if sclose > sl_level:
                self.log.info(
                    f"Exit SL: sclose={sclose:.2f} > sl_lvl={sl_level:.2f} "
                    f"(vwap={self.current_vwap:.2f}, entry={self.current_entry_sclose:.2f})"
                )
                self._exit_all("SL")
                self._prev_bar_sclose = sclose
                self._prev_bar_vwap = self.current_vwap
                return
        ...
        if self._fsm == STATE_IDLE:
            ...
            # crossunder of VWAP: prev bar was >= VWAP, current bar is below VWAP.
            # No PDL / PDH / prev-day filters — pure VWAP crossdown.
            crossed_under_vwap = (
                self._prev_bar_sclose is not None
                and self._prev_bar_vwap is not None
                and self._prev_bar_sclose >= self._prev_bar_vwap
                and sclose < self.current_vwap
            )

            if in_window and crossed_under_vwap:
                self._enter_short(sclose)

        # Update prev-bar refs for next iteration
        self._prev_bar_sclose = sclose
        self._prev_bar_vwap = self.current_vwap
    ...
    def _enter_short(self, sclose: float) -> None:
        ...
        self.log.info(
            ...
            f"sclose={sclose:.2f}, vwap={self.current_vwap:.2f}, "
            f"sl={self.current_vwap + self.sl_points:.2f}"
        )
    ...
    def _finalize_trade(self) -> None:
        ...
        trade_record = {
            ...
            "pnl": round(trade_pnl_points, 2),  # reporting.py expects "pnl" column
        }
        ...
        # If exit was a SHIFT, roll strikes to new ATM and re-open immediately
        # (bypass VWAP signal — the whole point of a shift is to stay in position).
    ...
    def on_order_filled(self, event: OrderFilled) -> None:
        ...
        # Find matching leg (first unfilled in correct direction) — current_legs is small (~30)
        matching = None
        for leg in self.current_legs:
        ...
        if is_sell:
            ...
            if self._fsm == STATE_PENDING_ENTRY and self._entry_fills >= self._entry_orders_submitted:
                ...
                # Capture ATM straddle (CE+PE at base_strike) → band size
                atm_ce = next(
    ...
    def get_last_lower_band(self) -> float | None:
        return self.last_lower_band_of_day
    ...
    def _roll_legs_to_new_atm(self) -> None:
        """Unsubscribe current legs, rebuild leg set around current ATM, subscribe new ones."""
        # (We don't unsubscribe existing subscriptions — Nautilus just keeps routing ticks;
        # but the _leg_by_id map is rebuilt so only the new legs contribute to sClose.)
    ...
    def _reset_vwap_state(self) -> None:
        """Reset the cumulative VWAP/SD state — new leg set = new aggregate series."""
        ...
        self._prev_bar_vwap = None
        self._current_minute_ns = 0
        self._bar_sclose = 0.0

    def _open_short_direct(self) -> None:
        """Open 30-leg short at the current leg set without waiting for VWAP signal.
        Used after SHIFT to maintain position at new strikes."""
        if not self.leg_instruments:
            return
        # Require at least ATM ± 3 on both sides
        if len(self.leg_instruments) < 14:
            return

        self.trade_num += 1
        # entry_sclose will be captured from first post-fill tick via _close_bar or initial
